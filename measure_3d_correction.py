...
pred = np.load('./data/SyRIP_3d_correction/correct_3D.npy')
...
with open('./outputs/angles_syrip_3d_correction.csv', 'w') as f:
    writer = csv.writer(f)
    writer.writerow(header)
    writer.writerows(AngleM3D)

header = ['UA_sym', 'LA_sym', 'UL_sym', 'LL_sym', 'UA_ang', 'LA_ang', 'UL_ang', 'LL_ang']
with open('./outputs/rating_syrip_3d_correction_thres28.csv', 'w') as f:
    writer = csv.writer(f)
    writer.writerow(header)
